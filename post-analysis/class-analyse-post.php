...
if ( ! class_exists( 'WPSEO_Video_Analyse_Post' ) ) {
	...
	class WPSEO_Video_Analyse_Post {
		 ...
		 * @var array
		 */
		protected static $alt_protocols = [];
		 ...
		 * @var array
		 */
		protected static $video_autoembeds = [];
		 ...
		 * @var array
		 */
		protected static $video_oembeds = [];
		 ...
		 */
		public function __construct( $content, $vid, $old_vid = [], $post = null ) {
		...
		public static function set_statics() {
			...
			if ( is_array( self::$active_plugins ) && self::$active_plugins !== [] ) {
				foreach ( self::$active_plugins as $name => $instance ) {
					...
					if ( is_array( $shortcodes ) && $shortcodes !== [] ) {
						foreach ( $shortcodes as $sc ) {
							self::$shortcodes[ $sc ][] = [ $instance, 'get_info_from_shortcode' ];
						}
					}
					unset( $shortcodes, $sc );
					...
					if ( is_array( $post_types ) && $post_types !== [] ) {
						foreach ( $post_types as $pt ) {
							self::$post_types[ $pt ][] = [ $instance, 'get_info_for_post_type' ];
						}
					}
					unset( $post_types, $pt );
					...
					if ( is_array( $meta_keys ) && $meta_keys !== [] ) {
						foreach ( $meta_keys as $key ) {
							self::$meta_keys[ $key ][] = [ $instance, 'get_info_from_post_meta' ];
						}
					}
					unset( $meta_keys, $key );
					...
					if ( is_array( $alt_protocols ) && $alt_protocols !== [] ) {
					...
					if ( is_array( $video_oembeds ) && $video_oembeds !== [] ) {
						/*
						 * {@internal Merge order reversed, if there is a handler name conflict
						 *            between plugins, defer to the more popular plugin which will
						 *            have been added first.}
		...
		protected function analyse() {
			...
			foreach ( $methods as $method ) {
				if ( is_callable( [ $this, $method ] ) ) {
					$vid = call_user_func( [ $this, $method ] );

					// Check for video.
					if ( $vid !== [] && ( isset( $vid['content_loc'] ) || isset( $vid['player_loc'] ) ) ) {
						$this->vid = array_merge( $this->vid, $vid );
						break;
					}
				}
		...
		protected function get_video_from_post_type() {
			...
			if ( ( is_array( self::$post_types ) && self::$post_types !== [] ) && ( is_object( $this->post ) && isset( self::$post_types[ $this->post->post_type ] ) ) ) {
		...
		protected function get_video_from_post_meta() {
			...
			if ( is_array( self::$meta_keys ) && self::$meta_keys !== [] && ! empty( $this->post->ID ) ) {
		...
		protected function get_normalized_meta_values( $key, $post_id ) {
			...
			if ( is_array( $meta_values ) && $meta_values !== [] ) {
				foreach ( $meta_values as $meta_value ) {
					...
					elseif ( is_array( $meta_value ) && $meta_value !== [] ) {
			 ...
			 */
			$real_values = array_map( [ $this, 'normalize_entities' ], $real_values );
		...
		protected function get_video_from_shortcode() {
			...
			if ( strpos( $this->content, '[' ) !== false && is_array( self::$shortcodes ) && self::$shortcodes !== [] ) {

				$old_shortcode_tags = $GLOBALS['shortcode_tags'];
				// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Reset at end of function.
					...
					foreach ( $matches as $match ) {
						...
						if ( $match[1] !== '[' && $match[6] !== ']' ) {
							...
							if ( is_array( $atts ) && $atts !== [] ) {
								$atts = array_map( [ $this, 'normalize_entities' ], $atts );
							}
				...
				// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Resetting the variable.
		...
		protected function get_video_from_auto_embeds() {
			...
			foreach ( $urls as $url ) {
				...
				$providerurl = $oembed->get_provider( $url, [ 'discover' => false ] );
			...
			if ( count( $matched_urls ) > 0 ) {
				...
				foreach ( $matched_urls as $match ) {
					...
					switch ( $url_info['domain'] ) {
						 ...
						 * @todo JRF -> this ought to be changed so that the $vid['url'] value is a normal value
						 * and we add another key for passing the content
						 */
						case 'screencast.com':
		 ...
		 * @return array
		 */
		protected function fix_sc_attributes( $atts = [] ) {
		 ...
		 * @return string
		 */
		protected function normalize_entities( $text ) {
			return trim( wp_kses_decode_entities( ent2ncr( wp_kses_normalize_entities( $text ) ) ) );
		}
		...
		public static function parse_url( $url, $component = -1 ) {
			...
			if ( strpos( $url, '/' ) !== 0 ) {
				...
				if ( is_array( $parsed_url ) && $parsed_url !== [] ) {
